...
class Category extends React.Component {
	...
	render() {
		//swipeOptions={{continuous: false}}默认是手移动才会动的
		var opt = {
			auto:2000,//一秒之后自动轮播
			callback:(index) => {//0.1.2 
				this.setState({
					index:index
				});
			}
		...
		return (
			<div id="home-category">
				...
				<div className="index-container">
					<ul>
						<li className={this.state.index === 0 ? "selected" : ''} data-index="0" ></li>
						<li className={this.state.index === 1 ? "selected" : ''} data-index="1" ></li>
						<li className={this.state.index === 2 ? "selected" : ''} data-index="2" ></li>
					</ul>
				</div>
			</div>
		);
